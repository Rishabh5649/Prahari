...
export default function CircularDetail() {
  ...
  const [activeOverrideForm, setActiveOverrideForm] = useState(null); // mapId or judgmentId
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true);
      const data = await getCircularDetail(id);
      setCircular(data);
    } catch (err) {
      console.error(err);
      ...
      const errorMsg = typeof detail === 'string'
        ...
        : (Array.isArray(detail)
            ...
            : 'Failed to load circular details.');
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <p className="text-sm font-medium text-zinc-600">Loading circular details...</p>
      </div>
    );
  }
  ...
  const topLevelMaps = circular.maps.filter((m) => !m.parent_map_id);
  const getChildren = (parentId) => circular.maps.filter((m) => m.parent_map_id === parentId);

  const renderMapRow = (mapItem, isChild = false) => {
    const children = getChildren(mapItem.id);
    const hasChildren = children.length > 0;
    const isExpanded = !!expandedParents[mapItem.id];

    return (
      <React.Fragment key={mapItem.id}>
        <TableRow className={`hover:bg-zinc-50/50 ${isChild ? 'bg-zinc-50/30' : ''}`}>
          <TableCell className="font-medium text-xs text-zinc-500">
            {hasChildren && (
              <button
                type="button"
                onClick={() => toggleParent(mapItem.id)}
                className="mr-1 hover:text-zinc-900 font-semibold focus:outline-none"
              >
                {isExpanded ? '▼' : '►'}
              </button>
            )}
          ...
          <TableCell className={`max-w-[450px] min-w-[250px] whitespace-normal break-words pr-4 text-xs font-medium text-zinc-900 ${isChild ? 'pl-4' : ''}`}>
            {mapItem.what}
          </TableCell>
          <TableCell className="text-xs text-zinc-700">{mapItem.department}</TableCell>
        ...
        {activeEvidenceForm === mapItem.id && (
          <TableRow className="bg-zinc-50/50">
            <TableCell colSpan={9} className="p-4 border-t border-zinc-200 whitespace-normal">
              <form
        ...
        {/* Judgment Reasoning Row (always shown if judgment exists and not form-active) */}
        {mapItem.latest_judgment && activeOverrideForm !== mapItem.id && (
          <TableRow className="bg-zinc-50/10 border-b border-zinc-100">
            <TableCell colSpan={9} className="py-2.5 px-4 text-left whitespace-normal">
        ...
        {/* Render Child Rows nested */}
        {hasChildren &&
          isExpanded &&
          children.map((childMap) => renderMapRow(childMap, true))}
      </React.Fragment>
    );
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 py-6 px-4">
      {/* Back link */}
      <div>
        <Link to="/" className="text-xs font-semibold text-zinc-500 hover:text-zinc-900">
      ...
      <Card className="border border-zinc-200 shadow-sm bg-white text-left">
        <CardHeader className="p-5 pb-2">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="space-y-1">
              ...
              <CardTitle className="text-xl font-bold text-zinc-950 leading-snug">
                {circular.title || 'Untitled Ingested Circular'}
              </CardTitle>
            </div>
            <div>
        ...
        <CardContent className="p-5 pt-0 text-xs text-zinc-500 space-y-2">
          {circular.source_url && (
            <div>
              Source URL:{' '}
              <a
          ...
          <div>Ingested at: {new Date(circular.ingested_at).toLocaleString()}</div>
        </CardContent>
      </Card>

      {/* MAPs Table */}
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-zinc-900 text-left">Mandatory Action Points</h2>
        {topLevelMaps.length === 0 ? (
          <div className="border border-dashed border-zinc-200 rounded-md p-8 text-center bg-zinc-50/50">
            <p className="text-sm text-zinc-500 font-medium">No requirements extracted from this circular.</p>
          </div>
        ) : (
            ...
            <Table>
              <TableHeader className="bg-zinc-50">
                <TableRow>
                  ...
                  <TableHead className="font-semibold text-xs text-zinc-700">Department</TableHead>
              ...
              <TableBody>{topLevelMaps.map((parentMap) => renderMapRow(parentMap, false))}</TableBody>
            </Table>
          </div>
        )}
      </div>
      ...
      {/* Audit Trail Link */}
      <div className="text-left pt-4 border-t border-zinc-200">
        <Link
          to={`/audit?entity_id=${circular.id}`}
          className="text-xs font-semibold text-zinc-600 hover:text-zinc-900 hover:underline"
        >
          View Circular Ingestion Audit Trail →
        </Link>
      </div>
    </div>
  );
